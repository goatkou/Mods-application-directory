# 📱 Liste de Mods Android / Android Mods List

Bienvenue dans la liste des mods Android ! Retrouvez ici quelques applications modifiées avec leurs fonctionnalités et options.

**Welcome to the list of Android mods! Here are some modified apps with their features and options.**

---

# YouTube ReVanced  

**FR : 🇫🇷**
YouTube ReVanced est une version modifiée de l'application YouTube officielle qui offre une expérience sans publicité, la possibilité de lire des vidéos en arrière-plan et de nombreuses options de personnalisation. Contrairement à l'application officielle, YouTube ReVanced permet de bloquer les publicités, de lire des vidéos en arrière-plan et d'offrir une interface épurée et intuitive.  

### 🎯 Fonctionnalités  
- 🚫 Suppression des publicités  
- 🔄 Lecture en arrière-plan  
- 🌓 Mode AMOLED pour économiser la batterie  
- 🎨 Personnalisation avancée de l'interface  
- 💬 Intégration des commentaires YouTube Shorts  
- 📱 Fonctionne sur les appareils root et non-root  

**Accessibilité :** (Root - Non-root)  

**EN : 🇬🇧** 
YouTube ReVanced is a modified version of the official YouTube app that provides an ad-free experience, background playback, and extensive customization options. Unlike the official app, YouTube ReVanced allows ad-blocking, background playback, and offers a clean and intuitive interface.  

### 🎯 Features  
- 🚫 Ad removal  
- 🔄 Background playback  
- 🌓 AMOLED mode for battery saving  
- 🎨 Advanced UI customization  
- 💬 YouTube Shorts comments integration  
- 📱 Works on rooted and non-rooted devices  

**Accessibility:** (Root - Non-root)  


[🌐 Web Page](https://revanced.app/)

---

# YouTube ReVanced Extended  

**FR : 🇫🇷**  
YouTube ReVanced Extended est une version améliorée de YouTube ReVanced, offrant encore plus de fonctionnalités et d'options de personnalisation avancées. Il intègre une meilleure gestion de la lecture en arrière-plan et des options supplémentaires pour optimiser votre expérience utilisateur.  

### 🎯 Fonctionnalités  
- 🚫 Suppression des publicités  
- 🔄 Lecture en arrière-plan améliorée  
- 🌓 Mode AMOLED pour économiser la batterie  
- 🎨 Personnalisation avancée de l'interface  
- 📌 Boutons personnalisables et plus d'options d'affichage  
- 🔧 Paramètres avancés pour une meilleure optimisation  
- 💬 Intégration améliorée des commentaires YouTube Shorts  
- 📱 Fonctionne sur les appareils root et non-root  

**Accessibilité :** (Root - Non-root)  

**EN : 🇬🇧**  
YouTube ReVanced Extended is an enhanced version of YouTube ReVanced, offering even more features and advanced customization options. It includes improved background playback management and additional settings to optimize your user experience.  

### 🎯 Features  
- 🚫 Ad removal  
- 🔄 Improved background playback  
- 🌓 AMOLED mode for battery saving  
- 🎨 Advanced UI customization  
- 📌 Customizable buttons and more display options  
- 🔧 Advanced settings for better optimization  
- 💬 Enhanced YouTube Shorts comments integration  
- 📱 Works on rooted and non-rooted devices  

**Accessibility:** (Root - Non-root)  


[📢 Telegram Group](https://t.me/revanced_extended)

---

# TikTok Private Plus  

**FR : 🇫🇷**  
TikTok Private Plus est une version modifiée de TikTok qui propose des fonctionnalités avancées pour améliorer votre expérience utilisateur. Il permet notamment de télécharger des vidéos sans filigrane et de supprimer les publicités. De plus, des fonctionnalités supplémentaires peuvent être ajoutées via une application complémentaire appelée **TikTok Plugin**.  

Une version plus simple, **TikTok Platinum**, est également disponible. Bien qu’elle offre moins de fonctionnalités, elle reste efficace pour profiter d’une expérience sans publicité.  

### 🎯 Fonctionnalités  
- 🚫 Suppression des publicités  
- 🎥 Téléchargement de vidéos sans filigrane  
- 🔧 Ajout de fonctionnalités via **TikTok Plugin**  
- 🛠️ Options avancées de personnalisation  
- 📱 Compatible avec les appareils **non-root**  

**Accessibilité :** (Non-root)  

**EN : 🇬🇧**  
TikTok Private Plus is a modified version of TikTok that provides advanced features to enhance your user experience. It allows you to download videos without watermarks and remove ads. Additionally, extra functionalities can be added through a complementary app called **TikTok Plugin**.  

A simpler version, **TikTok Platinum**, is also available. While it offers fewer features, it still ensures an ad-free experience.  

### 🎯 Features  
- 🚫 Ad removal  
- 🎥 Watermark-free video downloads  
- 🔧 Additional features via **TikTok Plugin**  
- 🛠️ Advanced customization options  
- 📱 Compatible with **non-root** devices  

**Accessibility:** (Non-root)  


[📢 Telegram Group](https://t.me/tiktokupdatez)

---

# xManager for Spotify

**FR : 🇫🇷**
xManager est un outil puissant permettant de gérer et d'installer différentes versions modifiées de Spotify sur Android. Il offre une interface intuitive et simplifiée, permettant aux utilisateurs d'accéder à des fonctionnalités avancées, notamment la suppression des publicités et des restrictions de lecture. Grâce à xManager, il est possible de télécharger et d'installer la version de Spotify la mieux adaptée à vos besoins, avec ou sans root.  

### 🎯 Fonctionnalités  
- 📌 Gestionnaire de versions Spotify (choisissez celle qui vous convient)  
- 🚫 Suppression des publicités  
- 🔓 Contournement des restrictions de lecture  
- 🛠️ Compatible avec les appareils root et non-root

**Accessibilité :** (Non-root)  

**EN : 🇬🇧**
xManager is a powerful tool for managing and installing various modified versions of Spotify on Android. It provides an intuitive and streamlined interface, allowing users to access advanced features such as ad removal and playback restrictions bypass. With xManager, you can download and install the most suitable version of Spotify for your needs, whether rooted or non-rooted.  

### 🎯 Features  
- 📌 Spotify version manager (choose the one that suits you)  
- 🚫 Ad removal  
- 🔓 Playback restriction bypass  
- 🛠️ Compatible with rooted and non-rooted devices  

**Accessibility:** (Non-root)  

[📢 Telegram Group](https://t.me/xManagerUpdates)

---


# MyInsta / InstaDM  

**FR : 🇫🇷**  
MyInsta et InstaDM sont des versions modifiées d'Instagram offrant des fonctionnalités avancées pour améliorer votre expérience. Elles permettent notamment de télécharger des stories et d’activer un **mode fantôme** pour consulter du contenu en toute discrétion.  

- **MyInsta** : Version stable avec moins de bugs, idéale pour une utilisation quotidienne.  
- **InstaDM** : Version bêta avec plus de fonctionnalités, mais pouvant contenir des bugs.  

### 🎯 Fonctionnalités  
- 📥 Téléchargement des stories et publications  
- 👻 Mode fantôme pour voir du contenu sans être détecté  
- 📨 Améliorations des messages directs (DM)  
- 🔧 Options avancées de personnalisation  
- 📱 Compatible avec les appareils **non-root**  

**Accessibilité :** (Non-root)  

**EN : 🇬🇧**  
MyInsta and InstaDM are modified versions of Instagram that provide advanced features to enhance your experience. They allow story downloads and include a **ghost mode** to view content without others knowing.  

- **MyInsta**: Stable version with fewer bugs, perfect for daily use.  
- **InstaDM**: Beta version with more features but potentially more bugs.  

### 🎯 Features  
- 📥 Story and post downloads  
- 👻 Ghost mode to view content undetected  
- 📨 Improved direct messaging (DM) features  
- 🔧 Advanced customization options  
- 📱 Compatible with **non-root** devices  


**Accessibility:** (Non-root)  



[📢 Telegram Group](https://t.me/instasmashrepo)

---

# SnapEnhance  

**FR : 🇫🇷**  
SnapEnhance est une version modifiée de Snapchat qui intègre des fonctionnalités avancées inspirées de l'abonnement **Snapchat+**, offrant une expérience enrichie aux utilisateurs. Parmi ces fonctionnalités, il est possible de voir et d’éditer les messages supprimés.  

L'application nécessite un **patchage via LSPatch** pour fonctionner correctement.  

### 🎯 Fonctionnalités  
- 👀 Voir les messages supprimés  
- ✏️ Modifier les messages envoyés  
- 🔓 Déblocage de certaines fonctionnalités premium de Snapchat+  
- 📱 Compatible avec les appareils **root** et **non-root**  

**Accessibilité :** (Root - Non-root)  

**EN : 🇬🇧**  
SnapEnhance is a modified version of Snapchat that integrates advanced features inspired by the **Snapchat+** subscription, providing an enhanced user experience. These features include the ability to view and edit deleted messages.  

The app requires **patching via LSPatch** to function properly.  

### 🎯 Features  
- 👀 View deleted messages  
- ✏️ Edit sent messages  
- 🔓 Unlock certain Snapchat+ premium features  
- 📱 Compatible with **rooted** and **non-rooted** devices  

**Accessibility:** (Root - Non-root)  


[Installation guide](https://github.com/rhunk/SnapEnhance/wiki/Installation-Guide)

[📢 Telegram Group](https://t.me/snapenhance)

---


# Xtra  

**FR : 🇫🇷**  
Xtra est une version modifiée de Twitch qui débloque des fonctionnalités exclusives pour améliorer l'expérience utilisateur. Il permet de **regarder des rediffusions payantes gratuitement** et d'accéder aux **emotes personnalisées** de plateformes comme **7TV, BTTV et FFZ**.  

### 🎯 Fonctionnalités  
- 📺 Accès gratuit aux rediffusions payantes  
- 😀 Intégration des emotes **7TV, BTTV, FFZ**  
- 🚫 Suppression de certaines limitations de l'application officielle  
- 📱 Compatible avec les appareils **non-root**  

**Accessibilité :** (Non-root)  

**EN : 🇬🇧**  
Xtra is a modified version of Twitch that unlocks exclusive features to enhance the user experience. It allows you to **watch paid replays for free** and access **custom emotes** from platforms like **7TV, BTTV, and FFZ**.  

### 🎯 Features  
- 📺 Free access to paid replays  
- 😀 Integration of **7TV, BTTV, FFZ** emotes  
- 🚫 Removal of certain limitations from the official app  
- 📱 Compatible with **non-root** devices  

**Accessibility:** (Non-root)  

[😺 Github Link](https://github.com/crackededed/Xtra)

---

# Piko  

**FR : 🇫🇷**  
Piko est une version modifiée de **Twitter (X)** qui ajoute des fonctionnalités exclusives pour une meilleure expérience utilisateur. Il permet notamment de **télécharger des vidéos directement depuis l’application** et d’accéder à certaines **options premium**.  

Le mod est disponible via **ReVanced Manager** ou sur leur **canal Telegram**.  

### 🎯 Fonctionnalités  
- 📥 Téléchargement de vidéos directement depuis l’application  
- 🚫 Suppression de certaines limitations de Twitter (X)  
- 🌟 Accès à certaines options premium  
- 🔧 Interface optimisée et personnalisable  
- 📱 Compatible avec les appareils **non-root**  

**Accessibilité :** (Non-root)  

**EN : 🇬🇧**  
Piko is a modified version of **Twitter (X)** that adds exclusive features for an enhanced user experience. It allows **video downloads directly from the app** and provides access to **certain premium options**.  

The mod is available via **ReVanced Manager** or their **Telegram channel**.  

### 🎯 Features  
- 📥 Download videos directly from the app  
- 🚫 Removal of certain Twitter (X) limitations  
- 🌟 Access to some premium options  
- 🔧 Optimized and customizable interface  
- 📱 Compatible with **non-root** devices  

**Accessibility:** (Non-root)  


[Installation guide (ReVanced Manager)](https://github.com/crimera/piko/blob/main/docs/images/manager.png)  

[📢 Telegram Group](https://github.com/crimera/piko)

---

# Revenge  

**FR : 🇫🇷**  
Revenge est une version modifiée de **Discord** qui offre des fonctionnalités avancées pour améliorer l'expérience utilisateur. Il permet notamment de **voir les messages supprimés**, de **voir les messages avant édition** et d'**accéder aux salons privés**, où seuls les noms des salons et la date du dernier message sont visibles.  

L'application nécessite un **patchage via Revenge Manager** et l'installation de **plugins** disponibles sur leur serveur Discord ou le [serveur Bunny](https://discord.gg/XjYgWXHb9Q).  

### 🎯 Fonctionnalités  
- 👀 Voir les messages supprimés  
- ✏️ Voir les messages avant édition  
- 🔐 Accéder aux salons privés (voir les noms des salons et la date du dernier message)  
- 🛠️ Installation de plugins via **Revenge Manager**  
- 📱 Compatible avec les appareils **root** et **non-root** ainsi que **iOS**  

**Accessibilité :** (Root - Non-root) et iOS  

**EN : 🇬🇧**  
Revenge is a modified version of **Discord** that provides advanced features to enhance the user experience. It allows you to **view deleted messages**, **see messages before editing**, and **access private channels**, where only the names of the channels and the date of the last message are visible.  

The app requires **patching via Revenge Manager** and installing **plugins** available on their Discord server or [Bunny's server](https://discord.gg/XjYgWXHb9Q).  

### 🎯 Features  
- 👀 View deleted messages  
- ✏️ See messages before editing  
- 🔐 Access private channels (view channel names and the date of the last message)  
- 🛠️ Install plugins via **Revenge Manager**  
- 📱 Compatible with **rooted** and **non-rooted** devices and **iOS**  

**Accessibility:** (Root - Non-root) and iOS  


[😺 Github Link](https://github.com/revenge-mod)

---

# NinjaGram  

**FR : 🇫🇷**  
NinjaGram est une version modifiée de **Telegram** qui propose des fonctionnalités premium pour améliorer l'expérience utilisateur. Parmi les fonctionnalités disponibles, il est possible de **voir les messages supprimés**.  

### 🎯 Fonctionnalités  
- 👀 Voir les messages supprimés  
- 🚫 Suppression des publicités  
- 🛠️ Options avancées de personnalisation  
- 📱 Compatible avec les appareils **non-root**  

**Accessibilité :** (Non-root)  

**EN : 🇬🇧**  
NinjaGram is a modified version of **Telegram** that offers premium features to enhance the user experience. One of the available features is the ability to **view deleted messages**.  

### 🎯 Features  
- 👀 View deleted messages  
- 🚫 Ad removal  
- 🛠️ Advanced customization options  
- 📱 Compatible with **non-root** devices  

**Accessibility:** (Non-root)  


[📢 Telegram Group](https://t.me/tele_ninja)
